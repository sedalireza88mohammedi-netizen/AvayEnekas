...
2. Fetch Kit details to determine version and available families:
   - **`fa` CLI:** First verify auth by running `fa whoami`. If logged in (or `FA_API_TOKEN` is set), run `fa kit show --kit-token <code>` to get the Kit's technology setting, and `fa kit family-styles --kit-token <code>` to get the kit's family-styles. If not authenticated, prompt the user to run `fa login` in a separate terminal first, or fall back below.
